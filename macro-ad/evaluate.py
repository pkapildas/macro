...
def evaluate_scores(scores, labels):
    ...
    if scores.ndim > 1:
        scores_flat = np.percentile(scores, 95, axis=-1).reshape(-1) if scores.ndim == 2 else scores.reshape(-1)
    else:
        scores_flat = scores
    ...
    # Clip extreme outliers at 99th percentile, then min-max normalize
    clip_val = np.percentile(scores_flat, 99)
    scores_flat = np.clip(scores_flat, scores_flat.min(), clip_val)
    ...
    # Auto-detect direction: some datasets have anomalies with lower error
    labels_binary = (labels_flat > 0.5).astype(int)
    try:
        auc_normal = roc_auc_score(labels_binary, scores_flat)
        auc_inverted = roc_auc_score(labels_binary, 1.0 - scores_flat)
        if auc_inverted > auc_normal:
            scores_flat = 1.0 - scores_flat
